...
class LHOnePage {
	...
	public function action_dispatcher(){
		...
		add_action('ct-options-content-templatesct-content-php', array($this, 'mb_ct_content'), 10, 1);
		add_action('ct-options-content-templatesct-social-php', array($this, 'mb_ct_content'), 10, 1);
		add_action('ct-options-content-templatesct-news-php', array($this, 'mb_ct_closer'), 10, 1);
		add_action('ct-options-content-templatesct-image_grid-php', array($this, 'mb_ct_closer'), 10, 1);
		add_action('ct-options-content-templatesct-image_grid_small-php', array($this, 'mb_ct_closer'), 10, 1);
		add_action('ct-options-content-templatesct-games-php', array($this, 'mb_ct_closer'), 10, 1);
	}
	...
	public function ct_info_callback(){
		...
		$response = array(
			"error" => true,
			"success" => false,
			"msg"	=> __("A random error occured.", "mimimi"),
		);
		...
		if($parent_template == self::$_onepage_template_file){
			...
			if($content_template){
				add_meta_box(
					...
					__("Content Template Options", "mimimi"),
	...
	public function mb_content_template_select($object, $box){
		...
		wp_nonce_field( basename( __FILE__ ), 'onepage_data_nonce' );
		?>

		<select class="ct-select widefat" name="lh_content_template">
			<option value=""><?php _e("No template", "mimimi"); ?></option>
			<?php
		...
		<div class="description">
			<?php
			if(isset($ct_headers) && is_array($ct_headers) && $ct_headers['thumbnail']){
			...
			<div class="desc-text">
				<?php
					if(isset($ct_headers) && is_array($ct_headers) && $ct_headers['description']){
						echo $ct_headers['description'];
					} else {
						_e("No content template is selected, but the parent page is a one page master. Be aware that this content might not be shown.", "mimimi");
					}
				?>
			</div>
		</div>
		<p class="clear">
			<?php edit_post_link(__("Edit One Page", "mimimi"), NULL, NULL, $object->post_parent); ?>
		</p>
		<?php
	...
	public function mb_ct_closer($ct_options) {
		global $post;
		$content_template = get_post_meta( $post->ID, '_lh_content_template', true );
		...
		<p>
			<label><?php _e("Claim text", "mimimi"); ?></label><br>
			<input type="text" value="<?php echo $ct_options['claim']; ?>" name="ct_options[claim]">
		</p>
		<p>
			<label><?php _e("'More' text", "mimimi"); ?></label><br>
			<input type="text" value="<?php echo $ct_options['more-text']; ?>" name="ct_options[more-text]">
		</p>
		<?php if($content_template == "content-templates/ct-games.php"): ?>
		<p>
			<label><?php _e("'Less' text", "mimimi"); ?></label><br>
			<input type="text" value="<?php echo $ct_options['less-text']; ?>" name="ct_options[less-text]">
		</p>
		<?php endif; ?>
		<?php if($content_template !== "content-templates/ct-games.php"): ?>
		<p>
			<label><?php _e("Link to", "mimimi") ?></label><br>
			<input type="text" value="<?php echo $ct_options['url']; ?>" name="ct_options[url]">
		</p>
		<?php endif; ?>
		<p>
			<ul class="mb-ct-closer-image cf ui-sortable clearfix">
				<li class="image-list-item">
					<img src="<?php echo wp_get_attachment_image_src($ct_options['image'], "thumbnail")[0]; ?>" class="image-thumbnail" />
					<input type="hidden" name="ct_options[image]" class="image-id-input" value="<?php echo $ct_options['image']; ?>" />
					<div class="delete">x</div>
				</li>
			</ul>
			<a href="#" class="mb-ct-closer-upload" data-title="<?php _e("Select Images", 'lh' ); ?>" data-action="<?php _e("Add Images to Selection", 'lh' ); ?>"><?php _e("Add Images", 'lh' ); ?></a>
		</p>
		<?php if($content_template !== "content-templates/ct-games.php"): ?>
		<p>
			<strong><?php _e('Background color', "mimimi"); ?></strong>
		</p>
		<p>
			<input type="text" class="widefat lh_color_picker" id="cp-bg-color" name="ct_options[bg]" value="<?php echo $ct_options["bg"]; ?>">
		</p>

		<p>
			<strong><?php _e('Font color', "mimimi"); ?></strong>
		</p>
		<p>
			<input type="text" class="widefat lh_color_picker" id="cp-font-color" name="ct_options[font]" value="<?php echo $ct_options["font"]; ?>">
		</p>
		<?php endif; ?>
		<?php
	}
	 ...
	 * @return void
	 */
	public function mb_ct_content($ct_options){
		...
		?>
			<p>
				<label for="ct_options_theme"><?php _e("Font Color", "mimimi"); ?></label><br />
				<input type="text" value="<?php echo $ct_options['color']; ?>" name="ct_options[color]" class="lh_color_picker">
			</p>
			<p>
				<label for="ct_options_theme"><?php _e("Background Color", "mimimi"); ?></label><br />
				<input type="text" value="<?php echo $ct_options['background-color']; ?>" name="ct_options[background-color]" class="lh_color_picker">
			</p>
			<p>
				<label for="ct_option_margin_bottom">
				<input type="checkbox" id="ct_option_margin_bottom" value="1" name="ct_options[margin-bottom]" <?php checked($ct_options['margin-bottom']); ?> ><?php _e("No margin bottom", "mimimi"); ?></label>
			</p>
		<?php
	}
	...
	public function box_save( $post_id, $post ) {
		...
		$this->save_post_meta($post_id, $post, 'onepage_data_nonce', 'lh_content_template', '_lh_content_template');
		$this->save_post_meta($post_id, $post, 'onepage_data_nonce', 'ct_options', '_ct_options');

		if(isset($_POST['ct_order']) && is_array($_POST['ct_order'])) {
	...
	// Helper function
	public function is_one_page($page_id = null){
		global $post;
...
if(!function_exists("is_one_page")){
	function is_one_page($page_id = null){
		global $_lh_onepage;
		return $_lh_onepage->is_one_page($page_id);
	}
}