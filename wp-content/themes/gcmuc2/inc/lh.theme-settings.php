...
class lh_theme_settings {
	...
	public function register_settings(){
		...
		$args = array(
			'id'			=> "main_settings",
			'title'			=> __("Main Settings", LANG_NAMESPACE),
			'page'			=> "lh_theme_settings",
			'description'	=> __("The main settings to control the theme.", LANG_NAMESPACE),
		);
		$settings_example_section = new lh_settings_section($args);


		$args = array(
			'id'				=> 'contact_field_code',
			'title'				=> __("Contact Field Code", LANG_NAMESPACE),
			'page'				=> 'lh_theme_settings',
			'section'			=> 'main_settings',
			'description'		=> __("The Shortcode for the WPCF7 Contact Field.", LANG_NAMESPACE),
...
class lh_settings_field {
	...
	public function output_callback(){
		...
		?>
			<fieldset>
				<input type="text" class="all-options" name="<?=$this->args['id']?>" id="<?=$this->args['id']?>" value="<?=esc_attr(get_option($this->args['id']))?>">
				<p class="description">
