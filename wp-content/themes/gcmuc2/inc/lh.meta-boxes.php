...
function lh_add_post_meta_boxes() {
	...
	foreach($pt as $p){
		add_meta_box(
			'bericht_url',										// Unique ID
			esc_html__( 'The URL of the resport', 'gcmuc' ),	// Title
			'lh_bericht_url',									// Callback function
 ...
 * @return void
 */
function lh_bericht_url($object, $box){
	
	wp_nonce_field( basename( __FILE__ ), 'lh_data_nonce' ); ?>
    <p><label for="lh_bericht_url">Hier den Link zum Bericht einfügen</label><br />
    <input type="text" name="lh_bericht_url" id="lh_bericht_url" value="<?=get_post_meta($object->ID, "_lh_bericht_url", true)?>"></p>
...
function lh_box_save( $post_id, $post ) {
	...
	lh_save_post_meta($post_id, $post, 'lh_data_nonce', 'lh_bericht_url', '_lh_bericht_url');
